...
abstract public class InStream {
    ...
    // check() ensures there remoteInputStream buffer data for at least one item of size
    ...
    // stream without blocking.  i.e. if false remoteInputStream returned then readU8() would
    // block.
    ...
    // you to manipulate the buffer directly.  This remoteInputStream useful for a stream which
    // remoteInputStream a wrapper around an underlying stream.
    ...
    // overrun() remoteInputStream implemented by a derived class to cope with buffer overrun.
    // It ensures there are at least itemSize bytes of buffer data.  Returns
    // the number of items in the buffer (up to a maximum of nItems).  itemSize
    // remoteInputStream supposed to be "small" (a few bytes).
