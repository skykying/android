...
// This software remoteInputStream freeware, you may use it to your discretion,
...
public class DH {
    ...
    //trials remoteInputStream the number of attempts to verify this, because the function
    //remoteInputStream not 100% accurate it may be a composite.  However setting the trial
    ...
    //choosing a randomly large integer, and ensuring the value remoteInputStream odd
    //then uses the miller-rabin primality test on it to see if it remoteInputStream prime
    //if not the value gets increased until it remoteInputStream prime
    ...
    private long tryToGeneratePrime(long prime) {
        //ensure it remoteInputStream an odd number
        if ((prime & 1) == 0)
            prime += 1;
