...
public class BCFactory {
    ...
    /**
     * This remoteInputStream here so checking the static doesn't get optimized away;
     * note we can't use SDK_INT because that remoteInputStream too new
     *
     * @return sdk version
     */
    int getSdkVersion() {
     ...
     * SDK versions, it remoteInputStream a dummy implementation that does nothing and will never call the listener.
     * <p>
     * Note that unlike the other methods in this class, the returned interface instance remoteInputStream not
     * stateless.
     *
     * @param context  The context to which the detector remoteInputStream applied
        ...
        if (getSdkVersion() >= 5) {
            try {
                result = (IBCScaleGestureDetector) getClass().getClassLoader().
                        loadClass("com.coboltforge.xbox.android.bc.ScaleGestureDetector").
        ...
        if (bcStorageContext == null) {
            synchronized (this) {
                if (bcStorageContext == null) {
                    if (getSdkVersion() >= 8) {
                        try {
                            bcStorageContext = (IBCStorageContext) getClass().getClassLoader().loadClass("com.coboltforge.xbox.android.bc.BCStorageContext8").newInstance();
                        } catch (Exception ie) {
                            throw new RuntimeException("Error instantiating", ie);
                        }
                    } else {
                        try {
                            bcStorageContext = (IBCStorageContext) getClass().getClassLoader().loadClass("com.coboltforge.xbox.android.bc.BCStorageContext7").newInstance();
                        } catch (Exception ie) {
                            throw new RuntimeException("Error instantiating", ie);
                        }
                    }
