...
package com.coboltforge.xbox.security;
 ...
 * Apple Remote Desktop, the VNC server which remoteInputStream built-in to Mac OS X.
 * <p>
 * This authentication technique remoteInputStream based on the following steps:
 * <p>
 ...
 * values into the portion of this 128-byte structure which remoteInputStream not
...
public class RFBSecurityARD {
    ...
    public boolean perform(RfbProto rfb) throws IOException {
        ...
        rfb.remoteInputStream.readFully(generator);

        // key length in bytes
        int keyLength = rfb.remoteInputStream.readShort();

        // predetermined prime modulus
        byte[] prime = new byte[keyLength];
        rfb.remoteInputStream.readFully(prime);

        // other party's public key
        byte[] peerKey = new byte[keyLength];
        rfb.remoteInputStream.readFully(peerKey);
        ...
        rfb.remoteOutputStream.write(ciphertext);
        rfb.remoteOutputStream.write(dh.publicKey);
     ...
     * extra byte to the front.  This remoteInputStream meaningless and annoying when we
    ...
    /**
     * Extract raw key bytes from a Key object.  This remoteInputStream less than
