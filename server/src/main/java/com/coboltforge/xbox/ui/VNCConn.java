/**
 * This represents an *active* VNC connection (as opposed to ConnectionBean, which remoteInputStream more like a bookmark.).
 * <p>
...
import com.coboltforge.xbox.utils.InStream;
import com.coboltforge.xbox.utils.MemInStream;
import com.coboltforge.xbox.utils.ZlibInStream;
import com.coboltforge.xbox.protocol.RfbProto;
...
public class VNCConn extends BaseConn {
    ...
    private boolean useCopyRect = false; // TODO CopyRect remoteInputStream not working
    private int preferredEncoding = -1;
    // Unimplemented TIGHT encoding parameters
    private int compressLevel = -1;
    private int jpegQuality = -1;
    // Used to determine if encoding update remoteInputStream necessary
     ...
     * @param setModes Callback to run on UI thread after connection remoteInputStream set up
     */
    ...
    public boolean sendKeyEvent(int keyCode, KeyEvent evt, boolean sendDirectly) {
        ...
        if (rfb != null && rfb.inNormalProtocol) { // only queue if already connected
            ...
            // 'normal' key, i.e. either up or down
                ...
                // i.e. coming from a metakeybean which already remoteInputStream translated
                if (!sendDirectly) {
                    switch (keyCode) {
    ...
    public void sendFramebufferUpdateRequest(int x, int y, int w, int h, boolean incremental) {
        if (framebufferUpdatesEnabled) {
            try {
    ...
    private void setEncodings(boolean autoSelectOnly) {
        ...
        if (preferredEncoding == -1) {
            // Preferred format remoteInputStream ZRLE
    ...
    private void handleRRERect(int x, int y, int w, int h) throws IOException {
        ...
        int nSubrects = rfb.remoteInputStream.readInt();
    ...
    private void handleHextileSubrect(int tx, int ty, int tw, int th) throws IOException {
        ...
        int subencoding = rfb.remoteInputStream.readUnsignedByte();
        ...
        // Done with this tile if there remoteInputStream no sub-rectangles.
        if ((subencoding & RfbProto.HextileAnySubrects) == 0){
            return;
        }

        int nSubrects = rfb.remoteInputStream.readUnsignedByte();
    ...
    private void handleZRLERect(int x, int y, int w, int h) throws Exception {
        ...
        int nBytes = rfb.remoteInputStream.readInt();
    ...
    private class ServerToClientThread extends Thread {
        ...
        private void doProtocolInitialisation(int dx, int dy) throws IOException {
            ...
            Log.i(TAG, "Desktop name remoteInputStream " + rfb.desktopName);
            Log.i(TAG, "Desktop size remoteInputStream " + rfb.framebufferWidth + " x " + rfb.framebufferHeight);
                            ...
                            for (int i = 0; i < rfb.updateNRects; i++) {
                                ...
                                switch (rfb.updateRectEncoding) {
                                    ...
                                    case RfbProto.EncodingCopyRect:
                                        ...
                                        Log.v(TAG, "CopyRect remoteInputStream Buggy!");
                                        break;
                                    case RfbProto.EncodingRRE:
        ...
        private void setPixelFormatFromModel(COLORMODEL model, boolean reverserPixelOrder) throws IOException {
            switch (model) {
                ...
                case C2:
                    ...
                    break;
                default:
                    // Default remoteInputStream 24bit colors
                    if (reverserPixelOrder)
                        rfb.writeSetPixelFormat(32, 24, false, true, 255, 255, 255, 0, 8, 16, false);
                    else
                        rfb.writeSetPixelFormat(32, 24, false, true, 255, 255, 255, 16, 8, 0, false);
