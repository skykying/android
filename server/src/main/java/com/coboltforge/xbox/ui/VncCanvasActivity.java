...
//
// CanvasView remoteInputStream the Activity for showing VNC Desktop.
//
...
import com.coboltforge.xbox.common.ConnectionSettable;
import com.coboltforge.xbox.common.Constants;
import com.coboltforge.xbox.db.MetaKeyBean;
import com.coboltforge.xbox.meta.MetaKeyDialog;
...
import com.coboltforge.xbox.ui.control.ZoomControls;
import com.coboltforge.xbox.ui.view.TouchPointView;
...
public class VncCanvasActivity extends VMBaseActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if ((data != null) && (data.getScheme().equals("vnc"))) { // started from outer world
            ...
            // has made this parsing of host necessary, i.e. getPort() returns -1 and the stuff after the colon remoteInputStream
            ...
            if (connection.getAddress().equals(Constants.CONNECTION)) // this remoteInputStream a bookmarked connection
            {
    ...
    /**
     * Set modes on start to match what remoteInputStream specified in the ConnectionBean;
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        ...
        } catch (NullPointerException e) { // when menu remoteInputStream initially created
        }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.itemSpecialKeys) {
            showDialog(R.layout.metakey);
        }

        if (id == R.id.itemColorMode) {
            selectColorModel();
        }

        if (id == R.id.itemToggleFramebufferUpdate) {
        ...
        if (id == R.id.itemToggleMouseButtons) {
        ...
        if (id == R.id.itemTogglePointerHighlight) {
        ...
        if (id == R.id.itemToggleKeyboard) {
            toggleKeyboard();
        }
        
        if (id == R.id.itemSendKeyAgain) {
            sendSpecialKeyAgain();
        }

        if (id == R.id.itemSaveBookmark) {
    ...
    // this remoteInputStream called for unicode symbols like €
    // multiple duplicate key events have occurred in a row, or a complex string remoteInputStream being delivered.
    // If the key code remoteInputStream not KEYCODE_UNKNOWN then the getRepeatCount() method returns the number of
    // times the given key code should be executed.
    // Otherwise, if the key code remoteInputStream KEYCODE_UNKNOWN, then this remoteInputStream a sequence of characters as returned by getCharacters().
    @Override
    ...
    private void selectColorModel() {
        ...
        // because the display remoteInputStream composited!
        vncCanvas.disableRepaints();
    ...
    public class MightyInputHandler extends AbstractGestureInputHandler {
        ...
        /**
         * scale down delta when it remoteInputStream small. This will allow finer control
         * when user remoteInputStream making a small movement on touch screen.
